# ちゅ〜るちゅーぶ デザインシステム design system
...
workspace package (`@workspace/churuchubu-design-system`): other artifacts depend
...
- The visual language uses white, ink black, and neutral gray as the base;
  rose-pink is reserved for important actions and states such as ticket
  purchase, selected navigation, live dates, and new content.
- The source app's supplied wordmark is retained at
  `docs/references/logos/churuchubu-logo.png` and shown in the preview Brand
  > Logo page.
...
- `src/preview/foundations.tsx` — token-driven Overview, Colors, Fonts, and Layout
  pages, plus the Brand Logo and Content hierarchy pages.
...
`@workspace/churuchubu-design-system/components/...`. Never use a `@/` alias inside
...
for `@workspace/churuchubu-design-system` and renders mockups using this package's
theme and components.

## Consuming this package

Never copy token values, component source, hooks, or these docs into a consuming
artifact. Add `@workspace/churuchubu-design-system` as a `workspace:*` dependency,
run `pnpm install`, and import directly from this package.

Read only the guides required by the current task:

- Building or styling web UI: `artifacts/churuchubu-design-system/docs/consuming-web.md`
- Building or styling Expo UI: `artifacts/churuchubu-design-system/docs/consuming-expo.md`
- Replacing an existing or scaffolded web theme/component library:
  `artifacts/churuchubu-design-system/docs/migrating-web.md`
- Replacing existing or scaffolded Expo theme/hooks/components:
  `artifacts/churuchubu-design-system/docs/migrating-expo.md`
