# Migrating Expo UI to ちゅ〜るちゅーぶ デザインシステム

Read `artifacts/churuchubu-design-system/docs/AGENTS.md` and
`artifacts/churuchubu-design-system/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/churuchubu-design-system/lib/native-theme` and
  `@workspace/churuchubu-design-system/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/churuchubu-design-system/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/churuchubu-design-system` and retains only
