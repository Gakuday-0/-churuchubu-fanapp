    ...
    <div className="space-y-4">
      <section className="rounded-xl border bg-card p-5 text-card-foreground">
        ...
        <p className="mt-3 max-w-2xl text-lg font-medium leading-relaxed">
          余白を活かした白・黒・グレーを基調に、ピンクは重要な操作と状態だけを引き立てる。
        </p>
        <p className="mt-2 text-sm text-muted-foreground">
          かわいさを残しながら、情報の優先度と読みやすさが伝わる静かなUIを目指します。
        </p>
      </section>
      <section className="rounded-xl border bg-card p-5 text-card-foreground">
    ...
    <div className="space-y-8 rounded-xl border bg-card p-6 text-card-foreground">
      ...
      <section className="space-y-4 border-t pt-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Color carries hierarchy rather than decorating every surface.
          </p>
        </div>
        <Guidelines
          items={[
            { kind: 'do', text: 'Use ink-black and neutral gray for headings, body copy, borders, and secondary actions.' },
            { kind: 'do', text: 'Reserve rose-pink for ticket purchase, selected states, live dates, new content, and other important actions.' },
            { kind: 'dont', text: 'Do not use pink as the default color for every icon, label, border, or navigation item.' },
          ]}
        />
      </section>
    </div>
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-xl border bg-card p-6 text-card-foreground">
        ...
        <div className="mt-5 rounded-xl border bg-white p-8">
          <img
            src={`${import.meta.env.BASE_URL}churuchubu-logo.png`}
            alt="ちゅ〜るちゅーぶ ロゴ"
            className="mx-auto h-auto w-full max-w-md object-contain"
          />
        </div>
        <p className="mt-4 text-sm text-muted-foreground">
          Use the supplied wordmark as the brand anchor. Keep supporting labels smaller and quieter than the logo.
        </p>
      </section>

      <section className="rounded-xl border bg-card p-6 text-card-foreground">
        <h2 className="font-semibold">Brand usage</h2>
        <div className="mt-4">
          <Guidelines
            items={[
              { kind: 'do', text: 'Give the logo generous breathing room and keep it visually dominant over supporting badges.' },
              { kind: 'do', text: 'Pair the logo with calm neutral surfaces so the mark remains recognizable.' },
              { kind: 'dont', text: 'Do not recreate, redraw, recolor, or compete with the supplied wordmark.' },
            ]}
          />
        </div>
      </section>
...
export function ContentGuidelinesPage() {
  return (
    <div className="space-y-6">
      <section className="rounded-xl border bg-card p-6 text-card-foreground">
        <h2 className="font-semibold">Hierarchy and tone</h2>
        <p className="mt-2 text-sm leading-relaxed text-muted-foreground">
          The fan experience should feel clear, trustworthy, and quietly polished.
          Let the content lead, and use color only when it helps people understand what matters.
        </p>
        <div className="mt-5">
          <Guidelines
            items={[
              { kind: 'do', text: 'Use black for headings, dark gray for explanatory copy, and light gray for supporting details.' },
              { kind: 'do', text: 'Make important actions explicit and easy to recognize, especially ticket purchase actions.' },
              { kind: 'dont', text: 'Do not make every label, icon, or secondary action compete for attention.' },
            ]}
          />
        </div>
      </section>
